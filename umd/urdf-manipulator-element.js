(function (global, factory) {
  ...
  */
  ...
  var URDFManipulator =
  /*#__PURE__*/
  function (_URDFViewer) {
    _inherits(URDFManipulator, _URDFViewer);

    _createClass(URDFManipulator, [{
      key: "disableDragging",
      get: function get() {
      ...
      key: "observedAttributes",
      get: function get() {
        return ['highlight-color'].concat(_toConsumableArray(_get(_getPrototypeOf(URDFManipulator), "observedAttributes", this)));
      }
    }]);

    function URDFManipulator() {
      var _getPrototypeOf2;

      var _this;

      _classCallCheck(this, URDFManipulator);

      for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
        args[_key] = arguments[_key];
      }

      _this = _possibleConstructorReturn(this, (_getPrototypeOf2 = _getPrototypeOf(URDFManipulator)).call.apply(_getPrototypeOf2, [this].concat(args))); // The highlight material
      ...
      var clickPoint = new THREE.Vector3(); // Reuseable variables

      var raycaster = new THREE.Raycaster();
      var delta = new THREE.Vector2();
      var plane = new THREE.Plane();
      var line = new THREE.Line3(); // The joint being manipulated

      var dragging = null;

      var toMouseCoord = function toMouseCoord(e, v) {
      ...
      var getCollisions = function getCollisions(m) {
        if (!_this.robot) return [];
        raycaster.setFromCamera(m, _this.camera);
        var meshes = [];

        _this.robot.traverse(function (c) {
          return c.type === 'Mesh' && meshes.push(c);
        });
      ...
      var findNearestJoint = function findNearestJoint(m) {
        var curr = m;
      ...
      var highlightLinkGeometry = function highlightLinkGeometry(m, revert) {
        var traverse = function traverse(c) {
          ...
          if (c.type === 'Mesh') {
            if (revert) {
              ...
              c.material = _this.highlightMaterial;
            }
      ...
      var intersect2 = new THREE.Vector3(); // Get the changed angle between mouse position 1 and 2
      // when manipulating target

      var getAngle = function getAngle(tg, m1, m2) {
        // TODO: Why is the constant negated?
        plane.normal.copy(tg.axis).transformDirection(tg.matrixWorld).normalize();
        plane.constant = -plane.normal.dot(clickPoint); // If the camera is looking at the rotation axis at a skewed angle

        temp.copy(_this.camera.position).sub(clickPoint).normalize();

        if (Math.abs(temp.dot(plane.normal)) < 0.2) {
          // distance to the clicked point
          var dist = temp.copy(clickPoint).sub(_this.camera.position).length() * 0.9; // Get the point closest to the original clicked point
          // and use that as center of the rotation axis

          temp.set(0, 0, 0).applyMatrix4(tg.matrixWorld);
          temp.addScaledVector(plane.normal, -plane.distanceToPoint(temp)); // Project out from the camera

          raycaster.setFromCamera(m1, _this.camera);
          intersect1.copy(raycaster.ray.origin).add(raycaster.ray.direction.normalize().multiplyScalar(dist));
          intersect1.sub(temp);
          raycaster.setFromCamera(m2, _this.camera);
          intersect2.copy(raycaster.ray.origin).add(raycaster.ray.direction.normalize().multiplyScalar(dist));
          intersect2.sub(temp);
          temp.crossVectors(intersect2, intersect1).normalize(); // Multiply by a magic number to make it feel good
          ...
          // and use that as center of the rotation axis
          temp.set(0, 0, 0).applyMatrix4(tg.matrixWorld);
          temp.addScaledVector(plane.normal, -plane.distanceToPoint(temp)); // project onto the plane of rotation
      ...
      var getMove = function getMove(tg, m1, m2) {
        var dist = temp.copy(clickPoint).sub(_this.camera.position).length();
        raycaster.setFromCamera(m1, _this.camera);
      ...
      el.addEventListener('mousedown', function (e) {
        if (_this.disableDragging) return;
        toMouseCoord(e, mouse);
        lastMouse.copy(mouse); // get the information on the clicked item
        // and set the dragged joint

        var target = getCollisions(mouse).shift();
        ...
        if (target) {
          ...
          if (dragging) {
            ...
            _this.dispatchEvent(new CustomEvent('manipulate-start', {
              bubbles: true,
              cancelable: true,
              detail: dragging.name
            }));
      ...
      _this._mouseMoveFunc = function (e) {
        toMouseCoord(e, mouse);
        delta.copy(mouse).sub(lastMouse); // Keep track of the hovered item. If an item is being
        ...
        if (dragging == null && _this.disableDragging === false) {
          var collision = getCollisions(mouse).shift() || null;
          var joint = collision && findNearestJoint(collision.object);
        ...
        if (hovered !== wasHovered) {
          if (wasHovered) {
            ...
            _this.dispatchEvent(new CustomEvent('joint-mouseout', {
              bubbles: true,
              cancelable: true,
              detail: wasHovered.name
            }));
          }
          ...
          if (hovered) {
            ...
            _this.dispatchEvent(new CustomEvent('joint-mouseover', {
              bubbles: true,
              cancelable: true,
              detail: hovered.name
            }));
        ...
        if (dragging !== null) {
          ...
          if (dragging.jointType === 'revolute' || dragging.jointType === 'continuous') {
            _delta = getAngle(dragging, mouse, lastMouse);
          } else if (dragging.jointType === 'prismatic') {
            _delta = getMove(dragging, mouse, lastMouse);
          } else {// Not supported
          }
      ...
      _this._mouseUpFunc = function (e) {
        if (dragging) {
          _this.dispatchEvent(new CustomEvent('manipulate-end', {
            bubbles: true,
            cancelable: true,
            detail: dragging.name
          }));
    ...
    _createClass(URDFManipulator, [{
      key: "connectedCallback",
      value: function connectedCallback() {
        _get(_getPrototypeOf(URDFManipulator.prototype), "connectedCallback", this).call(this);
      ...
      key: "disconnectedCallback",
      value: function disconnectedCallback() {
        _get(_getPrototypeOf(URDFManipulator.prototype), "disconnectedCallback", this).call(this);
      ...
      key: "attributeChangedCallback",
      value: function attributeChangedCallback(attr, oldval, newval) {
        _get(_getPrototypeOf(URDFManipulator.prototype), "attributeChangedCallback", this).call(this, attr, oldval, newval);
